...
app = firebase_admin.initialize_app(cred, {"projectId": os.environ["FIREBASE_PROJECT_ID"]}, name="dotly_api")

db = firestore.Client()

# create a collection if it doesn't exist
if not db.collection("users").document("admin").get().exists:
    db.collection("users").document("admin").set({"username": "admin", "password": os.environ["ADMIN_PASSWORD"]})
