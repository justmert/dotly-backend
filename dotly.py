...
# import github actor
from github.github_actor import (
    GithubActor,
)
...
class Flowana:
    ...
    def __init__(
        ...
        admin_sdk_path = os.path.join(
            base_dir,
            os.environ["FIREBASE_ADMIN_SDK_NAME"],
        )
    ...
    def delete_dummy_docs(
        self,
        collection_ref,
    ):
        dummy_docs = collection_ref.where(
            "dummy",
            "==",
            True,
        ).get()
    ...
    def run_protocol_update(
        ...
        self.pipeline.contruct_pipeline(
            protocol,
            collection_refs,
        )
